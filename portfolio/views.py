...
class ProjectDetailView(LoginRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        ctx["can_review"] = user.is_authenticated and user != p.owner and not ctx["user_has_reviewed"]
        return ctx
...
# --- NEW: review edit/delete

class ReviewOwnerOrProjectOwnerMixin(UserPassesTestMixin):
    """Allow the review author, the project owner, or staff to edit/delete."""
    def test_func(self):
        user = self.request.user
        ...
        return (
            ...
            or review.project.owner_id == user.id
        )


class ReviewUpdateView(LoginRequiredMixin, ReviewOwnerOrProjectOwnerMixin, UpdateView):
    model = Review
    form_class = ReviewForm
    template_name = "forms/form.html"
...
class ReviewDeleteView(LoginRequiredMixin, ReviewOwnerOrProjectOwnerMixin, DeleteView):
    model = Review
    template_name = "forms/confirm_delete.html"
